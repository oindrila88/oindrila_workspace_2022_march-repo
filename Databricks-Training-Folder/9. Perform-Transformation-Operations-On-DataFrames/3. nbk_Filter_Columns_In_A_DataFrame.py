...
# MAGIC %md
# MAGIC # Perform Filter Operation on DataFrame in Databricks
...
# Filter All Rows Where "c_birth_day", "c_birth_month", and "c_birth_year" are Valid - Bad Practice
...
df_FilterCustomerOnInvalidBirthDateBadPractice = df_ReadCustomerFileUsingCsv\
                              ...
                              .filter((col("c_birth_year").isNotNull()) & (col("c_birth_year") > 0) & (col("c_birth_year") <= datetime.date.today().strftime("%Y")))
...
# Filter All Rows Where "c_birth_day", "c_birth_month", and "c_birth_year" are Valid - Good Practice
from pyspark.sql.functions import col
...
df_FilterCustomerOnInvalidBirthDateGoodPractice = df_ReadCustomerFileUsingCsv\
                                                                              ...
                                                                              .filter(col("c_birth_year") <= datetime.date.today().strftime("%Y"))
...
df_FilterCustomerByCustomerSkBetweenValues = df_ReadCustomerFileUsingCsv\
                                                                    .filter(col("c_customer_sk").between(100, 400))
...
# DBTITLE 1,Filter Rows Using "SQL Expression" in "filter" Function With "expr ()" Method
from pyspark.sql.functions import col, trim, expr

df_FilterCustomerUsingSqlExpressionWithExpr = df_ReadCustomerFileUsingCsv\
                                                                .filter(expr("trim(c_first_name) == 'Brian' and trim(c_last_name) == 'Taylor'"))
display(df_FilterCustomerUsingSqlExpressionWithExpr)
