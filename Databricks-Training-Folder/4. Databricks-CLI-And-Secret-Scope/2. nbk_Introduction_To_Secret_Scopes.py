...
# MAGIC %md
# MAGIC # Secret Scopes in Databricks
# MAGIC * Topic: How to Create the "Databricks-Backed Secret Scope" and "Azure Key Vault-Backed Secret Scope"?
...
# MAGIC * A. Open the Databricks CLI in "Command Prompt" and Run the Command "databricks secrets create-scope --scope 'scope-name'" with the "Default Setting".
# MAGIC * B. Open the Databricks CLI in "Command Prompt" and Run the Command "databricks secrets create-scope --scope 'scope-name' --initial-manage-principal users" to "Create" a "Databricks-Backed Secret Scope" with "MANAGE" Permission for "All" the "Users" of the "Databricks Account".
...
# MAGIC * Open the Databricks CLI in "Command Prompt" and Run the Command "databricks secrets put --scope 'scope-name' --key 'key-name'" to "Create" a "Secret" in a "Databricks-Backed Secret Scope" using the "Databricks CLI".
...
# MAGIC * Open the Databricks CLI in "Command Prompt" and Run the Command "databricks secrets list --scope 'scope-name'" to Display the "List of Secrets" Present in a given "Secret Scope".
...
# MAGIC * Open the Databricks CLI in "Command Prompt" and Run the Command "databricks secrets delete --scope 'scope-name' --key 'key-name'" to "Delete" a "Secret" from a "Databricks-Backed Secret Scope".
...
# MAGIC * Example - "https:///adb-2428975292555881.1.azuredatabricks.net/#secrets/createScope"
...
# MAGIC %md
# MAGIC # Delete an "Azure Key Vault-Backed Secret Scope", or, "Databricks-Backed Secret Scope" from Azure Databricks CLI
# MAGIC * An "Azure Key Vault-Backed Secret Scope", or, "Databricks-Backed Secret Scope" can "Only" be "Deleted" from the "Databricks CLI" as there is no "dbutils.secret" Command to "Delete" a "Secret Scope".
# MAGIC * Open the Databricks CLI in "Command Prompt" and Run the Command "databricks secrets delete-scope --scope 'scope-name'" to "Delete" the "Azure Key Vault-Backed Secret Scope", or, "Databricks-Backed Secret Scope" Successfully.
