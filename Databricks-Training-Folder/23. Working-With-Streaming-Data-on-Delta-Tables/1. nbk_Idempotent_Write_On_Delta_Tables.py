...
# MAGIC %md
# MAGIC # What is "Idempotent Writes" on the "Delta Tables" in Databricks
...
# MAGIC %md
# MAGIC # What is "Idempotence"
# MAGIC * "<b>Idempotence"</b> is the "Property" of "Certain Operations" in "<b>Mathematics</b>" and "<b>Computer Science</b>" whereby those "Certain Operations" can be "Applied" "Multiple Times" "Without Changing" the "Result" beyond the "Initial Application".
...
# DBTITLE 1,Create a UDF that Returns Current Timestamp
...
def getCurrentTimestamp ():
    ...
    # Fetch and Return the "Current Timestamp"
    return datetime.timestamp(currentDateAndTime)
...
df_ReadCustomerFileUsingCsv.write.format("delta")\
                                 ...
                                 .option("txnVersion", int(getCurrentTimestamp()))\
...
# MAGIC %sql
# MAGIC SELECT COUNT(*) FROM retailer_db.tbl_DeltaCustomerWithIdempotentWrite;
