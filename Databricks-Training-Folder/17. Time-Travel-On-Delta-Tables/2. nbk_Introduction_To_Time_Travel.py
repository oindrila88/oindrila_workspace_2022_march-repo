...
# MAGIC %md
# MAGIC # Introduction to Time Travel of Delta Tables in Databricks
...
# MAGIC * A. "Re-creating" the "Analysis", "Reports", or "Outputs" (e.g., the "Output" of a "Machine Learning Model"). This could be useful for "Debugging" or "Auditing", especially in "Regulated Industries".
...
# MAGIC %sql
# MAGIC SELECT * FROM training.customers TIMESTAMP AS OF '2022-11-11T06:00:06.000Z'
...
timestamp_string = '2022-11-11T06:00:06.000Z'
df_TimeTravel = spark.read.format("delta").option("timestampAsOf", timestamp_string).table("training.customers")
...
# MAGIC %sql
# MAGIC SELECT * FROM training.customers VERSION AS OF 3
...
version = 3
df_TimeTravel = spark.read.format("delta").option("versionAsOf", version).table("training.customers")
display(df_TimeTravel)
...
latestVersion = spark.sql("SELECT MAX(version) FROM (DESCRIBE HISTORY training.customers)").collect()
df_LoadLatestVersion = spark.read.format("delta").option("versionAsOf", latestVersion[0][0]).table("training.customers")
...
# MAGIC %sql
# MAGIC SELECT * FROM training.customers@20221111060006000
...
df_TimestampOfDeltaTable = spark.read.format("delta").table("training.customers@20221111060006000")
...
# MAGIC %sql
# MAGIC SELECT * FROM training.customers@v3
...
df_VersionOfDeltaTable = spark.read.format("delta").table("training.customers@v3")
